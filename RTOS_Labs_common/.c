   ...
   uint32_t stackSize, uint32_t priority)
{
  // put Lab 2 (and beyond) solution here
	...
	status = StartCritical();
	if(NumCreated == 0){
		tcbs[NumCreated].next = &tcbs[0];
		tcbs[NumCreated].Id = NumCreated;	
		tcbs[NumCreated].SleepTime = 0;	
		tcbs[NumCreated].Priority = priority;
	}
	if(NumCreated <= NUMTHREADS-1){
		tcbs[NumCreated].next = &tcbs[0];
		tcbs[NumCreated-1].next = &tcbs[NumCreated];
		tcbs[NumCreated].Id = NumCreated;
		tcbs[NumCreated].SleepTime = 0;	
		tcbs[NumCreated].Priority = priority;
	...
	SetInitialStack(NumCreated); Stacks[NumCreated][STACKSIZE-2] = (int32_t)(task); // PC
  EndCritical(status);
