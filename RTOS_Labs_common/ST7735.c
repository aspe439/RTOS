...
void ST7735_Message(uint32_t  d, uint32_t  l, char *pt, int32_t value){
  ...
  OS_bWait(&STsema);
	...
	char* neg = "-";
  if(d == 0){ // top
		if(pt){ // if pointer is not empty 
			ST7735_DrawString(0, l, pt, ST7735_YELLOW ); //print the string
		}
		length = strlen(pt)+1; // find the length of the string (current cursor position)
	...
	if(value < 0 ) // if the number is negative
	{	
		ST7735_DrawString(length,l,neg, ST7735_YELLOW); //draw the negative sign
		length++; //record the current cursor position
